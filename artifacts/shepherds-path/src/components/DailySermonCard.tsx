...
/** Soft upsell only — never blocks playback. */
function showSermonUpsell(verseId: number): boolean {
  if (isProVerifiedLocally() || isOwnerPreviewActive()) return false;
  const ids = getSermonVerseIds();
...
export function DailySermonCard({ verseId, verseReference, reflectionContent, onSermonLoaded }: DailySermonCardProps) {
  ...
  const openPlayer = () => {
    if (!sermon) return;
    setShowPlayer(true);
    if (!isProVerifiedLocally() && !isOwnerPreviewActive()) {
      recordSermonVerseId(verseId);
    }
  };
                ...
                <div className="px-4 pb-4 pt-3 space-y-4">
                  {showUpsell && (
                    <div
                      ...
                      style={{ background: "rgba(124,58,237,0.12)", border: "1px solid rgba(124,58,237,0.2)", color: "rgba(255,255,255,0.65)" }}
                    >
                      You've explored {SERMON_LIMIT} messages.{" "}
                      <button
                        type="button"
                      ...
                      keeps a fresh one every day — you can still watch this clip below.
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 text-[11px] py-2 rounded-xl transition-colors"
                    style={{ color: "rgba(167,139,250,0.55)", background: "rgba(124,58,237,0.04)", border: "1px solid rgba(124,58,237,0.1)" }}
                  >
                    <ExternalLink className="w-3 h-3" />
                    Open in YouTube if the player doesn't start
                  </button>
        ...
        {showPlayer && sermon && (
          ...
          >
            ...
            >
              ...
              <div className="px-4 py-3 flex flex-col items-center gap-2" style={{ background: "rgba(0,0,0,0.4)" }}>
