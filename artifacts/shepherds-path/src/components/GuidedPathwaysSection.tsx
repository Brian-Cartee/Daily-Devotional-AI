...
export function GuidedPathwaysSection({ pathways, onSelect }: Props) {
  return (
          ...
          >
            ...
            <div className="relative z-10 flex items-start gap-3">
              <div className={`w-9 h-9 rounded-xl ${pathway.pillBg} flex items-center justify-center shrink-0`}>
                <MapPin className={`w-4 h-4 ${pathway.iconColor}`} />
              </div>
              <div className="flex-1 min-w-0">
                <span className={`text-[10px] font-bold uppercase tracking-widest ${pathway.pillText}`}>
                  7 days
                </span>
                <h3 className="text-[15px] font-bold text-foreground leading-tight mt-0.5">{pathway.title}</h3>
