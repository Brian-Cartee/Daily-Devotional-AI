    ...
    closeBundle() {
      ...
      if (fs.existsSync(outIndex)) {
        ...
        if (moduleTag) {
          ...
          const loaderScript = `<script type="module" data-sp-boot-marker="1">/* defer main bundle */</script>
<script>
(function(){
  var src=${JSON.stringify(moduleSrc)};
  var NATIVE_IMPORT_DELAY_MS=3200;
  ...
  function signalReactBootedToNative(){
    try{
      if(window.__spNativePostRaw){
        window.__spNativePostRaw(JSON.stringify({type:"react_booted",ts:Date.now()}));
      }
  ...
  setTimeout(function(){if(!window.__spMainModuleLoading)schedule();},3500);
  setTimeout(function(){if(!window.__spMainModuleLoading)schedule();},6000);
})();
</script>`;
