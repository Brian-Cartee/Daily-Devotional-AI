    ...
    try {
      ...
      try {
        ...
        await client.emails.send({
          from: fromEmail,
          to: input.email,
          subject: "Welcome to Daily Bread",
          html: `<div style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:32px;">
            ...
            <p style="color:#5c5248;line-height:1.7;">You're now subscribed to Daily Bread. Each morning you'll receive today's scripture, an encouragement message, and a link to explore it deeper with AI.</p>
          ...
          text: `Welcome${input.name ? `, ${input.name}` : ''}!\n\nYou're now subscribed to Daily Bread. Each morning you'll receive today's scripture and encouragement.\n\n${appUrl}`,
        });
      } catch (emailErr) {
